...
ASSET_PATH = "/Game/Botanicus/blueprints/BP_Item_SalePotSquare"
ASSET_NAME = "BP_Item_SalePotSquare"
ASSET_FOLDER = "/Game/Botanicus/blueprints"
PARENT_PATH = "/Script/Botanicus.BotanicusSalePotActor"
MESH_PATH = (
    "/Game/Botanicus/Items/itemsMesh/potDeVente/carré/"
    "pot_vente-carré"
)
...
def main():
    ...
    if mesh is None:
        raise RuntimeError("Square preparation-pot mesh missing: " + MESH_PATH)

    blueprint = unreal.EditorAssetLibrary.load_asset(ASSET_PATH)
    if blueprint is None:
        parent_class = unreal.load_class(None, PARENT_PATH)
        if parent_class is None:
            raise RuntimeError("Sale-pot parent class is unavailable")
    ...
    soil_half_x = max(1.0, outer_half_x * 0.72)
    soil_half_y = max(1.0, outer_half_y * 0.72)
    ...
    cdo.set_editor_property(
        "preparation_height_adjustment", -45.0 - bounds.min.z
    )
    cdo.set_editor_property(
    ...
    cdo.set_editor_property(
        "WorkbenchHeightAdjustmentSetting", -45.0 - bounds.min.z
    )
    cdo.set_editor_property(
    ...
    legacy_soil = cdo.get_editor_property("soil_visual")
    ...
    unreal.log_warning(
        "SQUARE_SALE_POT_READY {} mesh={} soil=({:.2f}, {:.2f}, {:.2f})".format(
